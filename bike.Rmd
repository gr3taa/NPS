...
attach(day)
n<- dim(day)[1]
result<-lm(cnt ~ hum +temp + windspeed+instant)
summary(result)
...
T0_x3 <- abs(summary(result)$adj.r.squared)
T0_x4<- abs(summary(result)$adj.r.squared)
regr.H01 <- lm(cnt~temp+windspeed+instant)
residuals.H01 <- regr.H01$residuals

regr.H02 <- lm(cnt~hum+windspeed+instant)
residuals.H02 <- regr.H02$residuals

regr.H03 <- lm(cnt~hum+temp+instant)
residuals.H03 <- regr.H03$residuals
...
grid=expand.grid(ist.grid,hum.grid,temp.grid)
names(grid)=c('ist','hum','temp')
pred_tp = predict(model_gam_reduced,
                  newdata = (grid))
open3d()
persp3d(hum.grid,temp.grid, pred_tp, col = 'grey30')
points3d(hum, temp, cnt, col = 'red', size = 5)
